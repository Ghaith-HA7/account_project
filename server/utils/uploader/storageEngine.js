 ...
 * @param {Function}cb
 */
function store(destination, filename, file, isPrivate, options, cb) {
			...
			sharp(tempPath)
				...
				.then((metadata) => {
					...
					if (err) {
						...
						//passed validation so store file from temp to local then delete temp when finish
...
/**
 *
 * @param {{filename: String || Function, destination: String, isPrivate: boolean, maxHeight: Number, maxWidth: Number }}opts
 * @constructor
 */
...
DiskStorage.prototype._handleFile = function _handleFile(req, file, cb) {
	...
	that.getFilename(req, file, function (err, filename) {
		...
		store(destination, filename, file, isPrivate, options, cb);
	});
};
